...
export class OrderService {
    ...
    async createOrder(orderDto:OrderDto, @Request() req)  {
        ...
        if(!cartItems || cartItems.length<1) throw new HttpException('Please provide cart items', HttpStatus.BAD_REQUEST);
        if(!delivery_fee) throw new HttpException('Please provide delivery fee', HttpStatus.BAD_REQUEST);
        let orderItems = [];
        let subTotal = 0;
        for(const item of cartItems){
        const validCart = await this.cartService.findById(item.cart, req)
        if(!validCart) throw new HttpException(`Cart with the given ID: ${item.cart} not found`, HttpStatus.NOT_FOUND)
   ...
   return {order, client_secret: order.client_secret};
    ...
    async getAll(queryStry: any, @Request() req): Promise<any> {
      ...
      let query = this.orderModel.find({}).populate({path:'user',select:'username email'}); 
    ...
    return {
      ...
      resultPerPage,
        ...
        const order = await this.orderModel.findOne({_id:id}).populate({path:'user',select:'username email'}).exec();
    
        if(!order) throw new HttpException('Order with the given ID not found', HttpStatus.NOT_FOUND);
        console.log(req.user.email, req.user.roles, order.user.email);

        const hasAdminRole = req.user.roles.includes(Role.Admin);

        if (order.user.email !== req.user.email && !hasAdminRole) throw new HttpException('You do not have permission to access this order', HttpStatus.UNAUTHORIZED)
        return order;
      };
    
        async currentUser(@Request() req): Promise<Order[] | undefined> {
        const order = await this.orderModel.find({user:req.user.id}).populate({path:'user',select:'username email'}).exec();
        if(!order || order.length === 0) throw new HttpException(`No Order has been placed by this current user: ${req.user.username}`, HttpStatus.NOT_FOUND);
        return order;
      };

      async approveOrder(id: string, @Request() req): Promise<Order | undefined> {
        const { status, payment_intentId } = req.body
        if(!status) throw new HttpException('Please provide the updated status of the order', HttpStatus.BAD_REQUEST);
        const order = await this.orderModel.findOne({_id:id}).populate({path:'user',select:'username email'}).exec();
        if(!order) throw new HttpException('The Order selected for approval does not exist', HttpStatus.NOT_FOUND);
        const approverId = req.user.id;
        const validApprover = await this.userService.findById(approverId)
        //console.log(validUser)
        if(!validApprover) throw new HttpException('Approver with the given ID not found', HttpStatus.NOT_FOUND);
      ...
      async delete(id: string, @Request() req): Promise<any> {
        const order = await this.orderModel.findOne({_id:id}).populate({path:'user',select:'username email'});
        console.log(order)
        if(!order) throw new HttpException('Order with the given ID not found', HttpStatus.NOT_FOUND);
        console.log(req.user.email, order.user.email);

        if(order.user.email !== req.user.email) throw new HttpException('You do not have permission to delete this order', HttpStatus.UNAUTHORIZED)
